...
| Web (Chrome) | `web/` | Same as desktop for people without a build; also the cheapest platform to build in CI (`flutter build web --release` in the `check` job). Persists through drift `WasmDatabase` (US-016; see "Data layer" below) — `web/sqlite3.wasm` (732 KB) + `web/drift_worker.js` (348 KB) add about 1.1 MB to `build/web` (39 MB total after `flutter build web --release`); both are fetched lazily by the worker only once the database first opens, not part of the initial `main.dart.js` payload (3.5 MB). |
